...
def main():
    ...
    Run the things.
    """
    folders_tocheck = cfg.bidsdir, cfg.logdir
    ...
    batch_jobs(cfg.subjectlist, cfg.dicomdir, cfg.configfile, cfg.bidsdir)
...
def batch_jobs(subject_list, dicomdir, configfile, niidir):
    ...
    for line in lines:
        ...
        session = entry.split(",")[2]
        subjectpath = os.path.join(dicomdir, subjectdir)
        if os.path.isdir(subjectpath):
            write_to_outputlog(subjectdir + os.linesep)
            if cfg.run_local:
                batch_cmd = 'dcm2bids -d {subjectpath} -s {session} -p {subject} -c {configfile} -o {niidir}  --forceDcm2niix --clobber'.format(subjectpath=subjectpath,  session=session, subject=subject, configfile=configfile,  niidir=bidsdir)
            else:
                batch_cmd = 'module load singularity; sbatch --job-name dcm2bids_{subjectdir} --partition=long --account=osl --nodes=1 --cpus-per-task=1 -o {logdir}/{subjectdir}_dcm2bids_output.txt -e {logdir}/{subjectdir}_dcm2bids_error.txt --wrap="singularity run -B {dicomdir} -B {niidir} -B {codedir} {image} -d {subjectpath} -s {session} -p {subject} -c {configfile} -o {niidir}  --forceDcm2niix --clobber"'.format(logdir=cfg.logdir, subjectdir=subjectdir, dicomdir=cfg.dicomdir, session = session, codedir=cfg.codedir, configfile=cfg.configfile, subject=subject, niidir=cfg.bidsdir, subjectpath=subjectpath, image=cfg.image)


                # batch_cmd = 'module load singularity; sbatch --job-name dcm2bids_{subjectdir} --account=osl --partition=long --nodes=1 --cpus-per-task=1 -o {logdir}/{subjectdir}_dcm2bids_output.txt -e {logdir}/{subjectdir}_dcm2bids_error.txt --wrap="singularity run -B {dicomdir} -B {niidir} -B {codedir} {image} -d {subjectpath} -p {subject} -c {configfile} -o {niidir}  --forceDcm2niix --clobber"'.format(logdir=cfg.logdir, subjectdir=subjectdir, dicomdir=cfg.dicomdir, codedir=cfg.codedir, configfile=cfg.configfile, subject=subject, niidir=cfg.bidsdir, subjectpath=subjectpath, image=cfg.image)

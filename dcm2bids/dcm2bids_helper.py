...
def main():
    ...
    Run the things.
    """
    dir_fullpaths = cfg.logdir, cfg.bidsdir, cfg.test_subject
    ...
    if os.path.isdir(cfg.dicomdir):
        ...
        if cfg.run_local:
            ...
            cmd = 'module load singularity; sbatch --job-name helper_{test_subject} --partition=short --account=osl --time=00:60:00 --mem-per-cpu=2G --cpus-per-task=1 -o {logdir}/{test_subject}_helper_output.txt -e {logdir}/{test_subject}_helper_error.txt --wrap="singularity exec -B {dicomdir} -B {parentdir} {image} dcm2bids_helper -d {dicomdir}/{test_subject} -o {bidsdir}"'.format(dicomdir=cfg.dicomdir, test_subject=cfg.test_subject, bidsdir=cfg.bidsdir, group=cfg.group, image=cfg.singularity_image, study=cfg.study, logdir=cfg.logdir, parentdir=cfg.parentdir)
...
def write_to_outputlog(message):
    ...
    """
    with open(cfg.outputloghelper, 'a') as logfile:
...
def write_to_errorlog(message):
    ...
    @param message:         Message to be printed to the log
    """
    with open(cfg.errorloghelper, 'a') as logfile:
        logfile.write(message + os.linesep)
    print(message)

...
bidsdir = os.path.join(os.sep, "projects", "osl", "xiy", "22Mo", "bids_data")
...
#        func_dir_path = os.path.join(bidsdir, subjectdir, 'func')
#        fmap_dir_path = os.path.join(bidsdir, subjectdir, 'fmap')
#        if os.path.isdir(func_dir_path):
#            func_niftis_partialpath = get_funcdir_niftis(func_dir_path)
    ...
    for subjectdir in subjectdirs:
        ...
        for timepoint in timepoints:
            ...
            if os.path.isdir(func_dir_path):
                ...
                if os.path.isdir(fmap_dir_path):
                    fmap_jsons = get_fmap_jsons(fmap_dir_path)
                    write_to_json(func_niftis_partialpath, fmap_jsons, fmap_dir_path, echo_time)
            else:
                continue
'''
for SAGE, fmap has two echotimes
def main():
    subjectdirs = get_subjectdirs()
    for subjectdir in subjectdirs:
        timepoints = get_timepoints(subjectdir)
...
# def get_funcdir_niftis(func_dir_path:str) -> list:
#    """
...
# for Mo, only one echo time
# added json_file['B0FieldIdentifier'] = "pepolar_fmap0" to identify methods of fmap calculation
def write_to_json(func_niftis_partialpath:list, fmap_jsons:list, fmap_dir_path:str, echo_time:str):
    for fmap_json in fmap_jsons:
        ...
        with open(json_path) as target_json:
            ...
            json_file['B0FieldIdentifier'] = "pepolar_fmap0"
            if include_echo_time:
                json_file['EchoTime'] = echo_time
